...
"""The following lines are to set the current dir to present dir"""
...
def build_interview_graph(llm, tavily_search=None):
    ...
    def generation_question(state:InterviewState):
        ...
        question = llm.invoke([SystemMessage(context = system_message)]+messages)
        #return the question formed
    ...
    def generate_answer(state: InterviewState):
        ...
        #Prompt using to answer questions
        system_message = GENERATE_ANSWERS.format(goals = analyst.persona, context = context)
        answer = llm.invoke([SystemMessage(content=system_message)]+messages)

        #To understand this message, we'll name it as as expert
        answer.name = "expert"
        #Apppending it with other part of the message
        return {"messages":[answer]}
    
    def save_interview(state:InterviewState):
        #Firstlty get the messages
        messages = state["messages"]
    ...
    def write_section(state: InterviewState):
        ...
        #Write sectionusing either the gathered source docs from interview (context) or the interview itself (interview)
    ...
    builder.add_node("ask_questions", generation_question)
...
class AutonomousReportGenerator:
    ...
    def create_analyst(self, state: GenerateAnalystsState):
        ...
        system_messages = CREATE_ANALYSTS_PROMPT.format(topic=topic,
    ...
    def write_conclusion(self, state: ResearchGraphState):
        ...
        topic = state["topic"]
        formatted_str_sections = "\n\n".join([f"{section}" for section in sections])
        #Summarizing into a final report
        instructions = INTRO_CONCLUSION_INSTRUCTIONS.format(topic = topic, formatted_str_sections = formatted_str_sections)
    ...
    def finalize_report(self, state: ResearchGraphState):
        ...
        if sources is not None:
            ...
            # Drafting the final report here
            final_report += "\n\n## Sources\n" + sources
    ...
    def build_graph(self):
        ...
        def initiate_all_interviews(state:ResearchGraphState):
            ...
            if not analysts:
                print("No analysts found - skipping interview")
                return END
            return[
                Send(
                    "conduct_interview",
        ...
        #Map each analyst into interview graph
