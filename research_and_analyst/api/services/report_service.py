...
class ReportService:
    ...
    def start_report_generation(self, topic: str, max_analysts: int):
        """Trigger the previously made pipeline"""
        try:
            ...
            return {"thread_id": thread_id, "message":"Pipeline initiated succesfully"}
    ...
    def submit_feedback(self, thread_id: str, feedback: str):
        """To update human feedback in graph state"""
        try:
            thread = {"configurable":{"thread_id":thread_id}}
            ...
            for _ in self.graph.stream(None, thread, stream_mode="values"):
                pass
            return {"message":"feedback updated succesfully"}
        except Exception as e:
