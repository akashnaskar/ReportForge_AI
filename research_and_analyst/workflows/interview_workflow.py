...
class InterviewGraphBuilder:
    """
    A class reesponsioble for constructing and managing the interview graph workflow. Handles the process of:
    1.Analyst generating questions
    2.Performing relevany web search
    3.Expert generating answers
    4.Saving the interview transcript
    5. Write summarised report section
    """
    ...
    def _generate_questions(self, state: InterviewState):
        """Generates the first question based on the analysts persona"""
        ...
        try:
            ...
            self.logger.info("Question generated succesfully", question_preview= question.content[:200])
            return {"messages":[question]}
        except Exception as e:
            self.logger.error("Error generating the analyst question", error=str(e))
    ...
    def _search_web(self, state: InterviewState):
        """Generate a structured search query and perform Tavily Search"""
        try:
    ...
    def _generate_answer(self, state:InterviewState):
        """
        Use the analysts context to generate an expert answer
        """
        analyst = state["analyst"]
        messages = state["messages"]
        ...
        try:
            ...
            answer.name = "expert"
            self.logger.info("Expert answer egenerated suvvesfully ", preview = answer.content[:200])
    ...
    def build(self):
        """
        Constreuct tme mmain graph and compile the LangGraph interview workflow
        """
        try:
            self.logger.info("Building Interview Graph workflow")
            builder = StateGraph(InterviewState)

            builder.add_node("ask_questions", self._generate_questions)
            ...
            builder.add_edge("ask_questions","search_web")
            ...
            self.logger.info("Interview Graph compiled succesfully")
            return graph
        
        except Exception as e:
            self.logger.error("Error building intervierw graph", error=str(e))
