...
from research_and_analyst.schemas.models import (
    Perspectives, GenerateAnalystsState, ResearchGraphState,
)
...
class AutonomousReportGenerator:
    ...
    def create_analyst(self, state: GenerateAnalystsState):
        """Generate analyst persona based opn topic and state"""
        topic = state["topic"]
    ...
    def human_feedback(self):
        """Pause node for human_analyst_feedback"""
        try:
            self.logger.info("Awaiting human feedback")
        except Exception as e:
            self.logger.error("Error during feedback stage", error= str(e))
            raise ResearchAnalystException("Human feedback noide failed", e)
    ...
    def write_report(self, state: ResearchGraphState):
        ...
        try:
            ...
            self.logger.info("Reporty written succesfully")
    ...
    def finalize_report (self, state: ResearchGraphState):
        """Combine together the intro, content, and conclusion into final report"""
        try:
            ...
            self.logger.error("Error fianlizing report", error = str(e))
    ...
    def save_report(self, final_report : str, topic:str, format:str ="docx"):
        """Save the document as docx or pdf,each in its own subfolder"""
        try:
            ...
            self.logger.info("Report saved succesfully", path=file_path)
            return file_path
...
if __name__=="__main__":
    try:
        ...
        if final_report:
            reporter.logger.info("Report generated succesfully")
